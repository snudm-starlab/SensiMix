...
"""PyTorch SensiMix model for inference. """
...
from .quantized_modules import mix_Linear, BinarizeLinear_inference
...
class MPQBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0).half()
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size).half()
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size).half()

        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = MPQBertLayerNorm(config.hidden_size, eps=config.layer_norm_eps).half()
...
class MPQBertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = nn.Linear(config.hidden_size, self.all_head_size).half()
        self.key = nn.Linear(config.hidden_size, self.all_head_size).half()
        self.value = nn.Linear(config.hidden_size, self.all_head_size).half()
    ...
    def forward(
        ...
        hidden_states = hidden_states.half() #sensimix
        ...
        if encoder_hidden_states is not None:
            encoder_hidden_states = encoder_hidden_states.half() #sensimix
...
class MPQBertSelfOutput(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attention_head_size = config.attention_head_size
        self.all_head_size = self.num_attention_heads * self.attention_head_size
        self.dense = nn.Linear(self.all_head_size, config.hidden_size).half() #sensimix
        self.LayerNorm = MPQBertLayerNorm(config.hidden_size, eps=config.layer_norm_eps).half() #sensimix
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        input_tensor = input_tensor.half() #sensimix
...
class MPQBertSelfAttentionp(nn.Module):
    def __init__(self, config):
        ...
        self.query = nn.Linear(config.hidden_size, self.all_head_size).half()#sensimix
        self.key = nn.Linear(config.hidden_size, self.all_head_size).half()#sensimix
        self.value = nn.Linear(config.hidden_size, self.all_head_size).half()#sensimix
...
class MPQBertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([MPQBertLayer(config).half() if _<config.num_8bit_layers else MPQBertLayer_MP_encoder(config) for _ in range(config.num_hidden_layers)])
...
class MPQBertModel(MPQBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.pooler = MPQBertPooler(config).half() #sensimix
    ...
    Examples::
        ...
        embedding_output = embedding_output.half() #sensimix
        ...
        sequence_output = sequence_output.half() #sensimix
...
class MPQBertForSequenceClassification_inference(MPQBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = MPQBertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, self.config.num_labels).half()
    ...
    def forward(
        ...
        pooled_output = pooled_output.half() #sensimix
